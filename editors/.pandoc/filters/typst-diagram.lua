...
local mermaid_config = os.getenv("HOME") .. "/.config/mermaid/typst-config.json"
    ...
    return with_temporary_directory("diagram", function (tmpdir)
      return with_working_directory(tmpdir, function ()
        ...
        pipe(
          ...
          {"--configFile", mermaid_config, "--scale", "2", "--pdfFit", "--input", infile, "--output", outfile},
          ''
        )
...
local function get_engine (name, engopts, format)
  ...
  -- Force using png for mermaid diagrams, since svg output is not working properly with typst.
  local requested = engopts['mime-type'] or engopts['mime-types']
  local mime_type
  if requested then
    mime_type = format:best_mime_type(
      engine.mime_types,
      requested
    )
  elseif name == "mermaid" then
    mime_type = "image/png"
  else
    mime_type = format:best_mime_type(engine.mime_types)
  end
